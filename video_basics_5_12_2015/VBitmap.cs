...
namespace testmediasmall
{
    ...
    public class VBitmap
    {
        ...
        public void FromFrame(byte [,,] pdata)
        {
            if (pdata == null) return;
            SetDimensions(pdata.GetLength(1), pdata.GetLength(0));
            for (int j = 0; j < ResY; ++j)
            {
                for (int i = 0; i < ResX; ++i)
                {
                    Pixels[j, i].R = pdata[j, i,2]/255.0;
                    Pixels[j, i].G = pdata[j, i, 1] / 255.0;
                    Pixels[j, i].B = pdata[j, i, 0] / 255.0;
                    Pixels[j, i].A = 1.0;
                }
            }
        }
