...
namespace testmediasmall
{
    public class Screen
    {
        ...
        private void ChangeMode()
        {
            ...
            if (mode == Mode.sequence) {mode = Mode.pan;}
            //if (mode == Mode.pan){mode = Mode.zoom;}
            if (mode == Mode.zoom){mode = Mode.color;}
            if (mode == Mode.color) { mode = Mode.sequence; }
            //if (mode == Mode.none) { mode = Mode.sequence; }
            //else { mode = Mode.none; }
            //mode = Mode.sequence;
        }
        ...
        void DoPan()
        {
            MediaWindow.Screens[0].ison = false;
            MediaWindow.Screens[2].ison = false;
        ...
        void DoColor()
        {
            ...
            else if (deviation < 20 && deviation > 0.00000000001 && framecount > transitionInterval)
            {
                //projF = projGM;
                iscolor = true;
                colorcount = 0;
                threshold = 0;
                newFrame = MediaWindow.domiHueTransition(cframe, false);
        ...
        void DisableColor() 
        {
            for (int i = 0; i < MediaWindow.screenCount; i++)
            {
                if (MediaWindow.Screens[i].iscolor) 
                {
                    MediaWindow.Screens[i].iscolor = false;
                   MediaWindow.Screens[i].colorcount = 0;
                }
                else
                {
                    MediaWindow.Screens[i].iscolor = false;
